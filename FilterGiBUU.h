...
//--------------------FilterGiBUU------------------
//
// Code for conversion of a GiBUU tree to the GENIE format
// used by the wider e2a analysis code base
// Directly derived from the ROOT to GENIE conversion code
// (FilterData)
//
#ifndef FILTERGIBUU_H
#define FILTERGIBUU_H
...
public :
   ...
   std::string fpath;      // The path to data files
   ...
   // Declaration of leaf types - this will change to the contents of the GiBUU formatted tree
   UChar_t         npart;
   UInt_t          runnb;
   UInt_t          evntid;
   UChar_t         evstat;
   ...
   FilterGiBUU(std::string a_path,std::string a_target,std::string a_beam_en, TTree *tree=0);
   virtual ~FilterGiBUU();
...
FilterGiBUU::FilterGiBUU(std::string a_path, std::string a_target, std::string a_beam_en, TTree *tree) : fChain(0)
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
     ...
     fTorusCurrent = 0;
      ...
      // of trees.
      TChain * chain = new TChain("ch","FilterGiBUU");
      //chain->Add(Form("/home/stuart/e4nu/data/e2a_%s_%s_v1/*.root/h10", ftarget.c_str(), fbeam_en.c_str()));  
      //chain->Add(Form("%s/e2a_%s_%s_v1/*.root/h10", fpath.c_str(), ftarget.c_str(), fbeam_en.c_str()));   //
      chain->Add(Form("%s/GiBUU_%s_%s/*.root/h10", fpath.c_str(), ftarget.c_str(), fbeam_en.c_str()));   //
...
FilterGiBUU::~FilterGiBUU()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t FilterGiBUU::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t FilterGiBUU::LoadTree(Long64_t entry)
{
...
void FilterGiBUU::Init(TTree *tree)
{
...
void FilterGiBUU::Show(Long64_t entry)
{
...
Int_t FilterGiBUU::Cut(Long64_t entry)
{
