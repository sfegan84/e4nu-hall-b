...
public :
   ...
   e2a_eppi_v1(std::string, std::string, std::string,TTree *tree=0);
   virtual ~e2a_eppi_v1();
...
e2a_eppi_v1::e2a_eppi_v1(std::string a_path, std::string a_target,std::string a_beam_en, TTree *tree) : fChain(0)
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
     ...
     fTorusCurrent = 0;
      ...
      //chain->Add(Form("/home/stuart/e4nu/data/e2a_%s_%s_v1/*.root/h10", ftarget.c_str(), fbeam_en.c_str()));   //local path to test data files. data path as a command line argument is a to-do feature
...
e2a_eppi_v1::~e2a_eppi_v1()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t e2a_eppi_v1::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t e2a_eppi_v1::LoadTree(Long64_t entry)
{
...
void e2a_eppi_v1::Init(TTree *tree)
{
...
void e2a_eppi_v1::Show(Long64_t entry)
{
...
Int_t e2a_eppi_v1::Cut(Long64_t entry)
{
