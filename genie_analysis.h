...
public :
   ...
   std::string fpath;       // The path to data
   ...
   genie_analysis(std::string, std::string, std::string, int, int ,TTree *tree=0);
   ...
   virtual void     Analyse(Int_t choice);
...
genie_analysis::genie_analysis(std::string a_path,std::string a_target,std::string a_beam_en, int number_rotations, int choice,TTree *tree) : fChain(0)
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
	...
	//chain->Add(Form("/w/hallb-scifs17exp/clas/claseg2/apapadop/eresmaid_%s_%s_hA2018_LFG_FSI_NoRadCorr_3M.root", ftarget.c_str(), fbeam_en.c_str())); 
		chain->Add(Form("%s/eresmaid_%s_%s_hN2018_LFG_FSI_NoRadCorr_3M.root", fpath.c_str(), ftarget.c_str(), fbeam_en.c_str())); 
      ...
      if (fchoice == 0) {
	//chain->Add(Form("/w/hallb-scifs17exp/clas/claseg2/apapadop/genie_filtered_data_e2a_ep_%s_%s_neutrino6_united4_radphot_test_100M.root",ftarget.c_str(), fbeam_en.c_str()));
	//chain->Add(Form("/home/stuart/e4nu/code/e4nu_1p1pi/genie_filtered_data_e2a_ep_%s_%s_test_100M.root",ftarget.c_str(), fbeam_en.c_str()));
	chain->Add(Form("%s/genie_filtered_data_e2a_eppi_%s_%s_test_100M.root",fpath.c_str(),ftarget.c_str(), fbeam_en.c_str()));
      }
