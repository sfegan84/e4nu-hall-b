...
void FilterGiBUU::Loop()
{
  int CounterEvents = 0;
	...
	TFile *file_in3= new TFile(Form("FiducialsCorrections/vz_v6/vz_%s_%s.root",ftarget.c_str(),fbeam_en.c_str()));;  //path altered for ROOT v6 compatible vertex correction TF1
	...
	TFile *file_out = new TFile(Form("/home/stuart/e4nu/code/e4nu_1p1pi/genie_filtered_GiBUU_e2a_eppi_%s_%s_test_100M.root",ftarget.c_str(),fbeam_en.c_str()), "Recreate");
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) {
		...
		//int nb = GetEntry(jentry); //is this actually unused?
		if (ientry < 0) break;
